...
/**
 * Tabela completa para modo leitura
 */
export function ReadingTable({
  ...
  const readingTypes = ['manga', 'manhwa', 'manhua', 'novel', 'book'];
  const filtered = mediaList.filter(m => readingTypes.includes(m.media_type));
      ...
      <table className="w-full border-collapse" role="grid">
        <thead>
          <tr className="border-b border-zinc-800 text-left text-xs font-medium text-zinc-400 uppercase tracking-wider">
            <th className="px-3 py-2 w-20">Capa</th>
            <th className="px-3 py-2">Título</th>
            <th className="px-3 py-2 w-32">Progresso</th>
            <th className="px-3 py-2 w-36">Status</th>
            <th className="px-3 py-2 w-20">Nota</th>
          </tr>
        </thead>
        <tbody>
          {filtered.map((media) => (
            <ListRow
              key={media.id}
              media={media}
              onUpdate={(updates) => onUpdate(media.id, updates)}
              variant="compact"
            />
          ))}
          {filtered.length === 0 && (
